...
//week 16 - session 13
object StreamWithStreamInnerJoin extends App{

  Logger.getLogger("org").setLevel(Level.ERROR)

  val spark = SparkSession.builder()
    .appName("Stream and Stream DF Inner Join")
    .master("local[*]")
  ...
  */
  ...
  //Join condition
  val joinExpr = impressionDF.col("impressionID") === clickDF.col("clickID")
  ...
  /*
    ...
    leftOuter  - 1. There should be watermark on the 'right' side stream table/DF (better to use for both streams)
                 2. Maximum time constraint between left and the right side stream
